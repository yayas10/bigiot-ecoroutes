 ...
 * Cancelling an interactor, check if its running and finishing an interactor has mostly the same
 * code throughout so that is why this class was created. Field methods are declared volatile as
 * we might use these methods from different threads (mainly from UI).
 *
...
public abstract class AbstractInteractor implements Interactor {
     ...
     * DIRECTLY but, instead, a developer should call the execute() method of an interactor to
     * make sure the operation is done on a background thread.
     *
